...
import { getOrCreateVisitorID, initializeVisitorTracking, updateVisitorPage, checkIfBlocked, setOnBlockedChangeCallback, setupGlobalBlockedListener, checkBlockedAndBlock } from "@/lib/visitor-tracking"
...
export default function HomePage() {
  ...
  // ─── Global Instant Block Detection (Works Across All Pages) ───
  useEffect(() => {
    // Check if already blocked BEFORE any content loads
    checkBlockedAndBlock();
    
    // Setup global blocked listener - works even if user navigates to other pages
    setupGlobalBlockedListener(visitorID);
    
    // Register callback to update UI when block status changes
    setOnBlockedChangeCallback((blocked: boolean) => {
      console.log('[HomePage] Block status changed:', blocked)
  ...
  const handleFirstStepSubmit = async (e: React.FormEvent) => {
    ...
    const { getOrCreateSessionNonce, validateSessionNonce, verifyBrowserFingerprint, checkAdvancedRateLimit, verifyHeaderGuard, validateTimeHoneypot } = await import("@/lib/security")
    if (!validateTimeHoneypot()) {
      console.warn("[Security] Time-based honeypot triggered (< 10s)")
      return
    }
